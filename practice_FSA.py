    ...
    def print_to_manim(self,input_string):
        ...
        for symbol in input_string:
            if (current_state, symbol) in self.delta:
                ...
                print(f"\t\tarrow{current_state}{end_state}.next_to(arrow_{current_state}_{end_state}, UP*1)", file=out)
                print(f"\t\tself.play(Create(arrow_{current_state}{end_state}), Write(arrow_label_{current_state}_{end_state}))", file=out)
                current_state = end_state
                print(f"\t\tself.wait(0.5)", file=out)
            else:
                print("Transition not defined for current state and symbol")
                break

        if current_state in self.finalstates:
            print(f"\t\tfinal_state_{current_state}.set_fill(GREEN, opacity=0.5)", file=out)
            print("\t\ta = Text(f'{input_string} is accepted', font_size=13).next_to(c{current_state}, DOWN)",file=out)
            print(f"\t\tself.play(Write(a))", file=out)
        else:
            print(f"\t\tc{self.initialstate}.set_fill(RED, opacity=0.5)", file=out)
            print("\t\tb = Text(f'{input_string} is not accepted', font_size=13).next_to(c{self.initialstate}, DOWN)",file=out)
